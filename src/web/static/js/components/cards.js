...
/**
 * Renders skeleton loading for results info bar.
 * @returns {string} The skeleton HTML
 */
export function renderResultsInfoSkeleton() {
    return `
        <div class="results-info-skeleton">
            <div class="skeleton-shimmer" style="height: 1rem; width: 220px;"></div>
            <div class="skeleton-shimmer" style="height: 1.5rem; width: 80px; border-radius: 999px;"></div>
        </div>
    `;
}

/**
 * Renders skeleton loading cards for search results.
 * @param {number} count - Number of skeleton cards to show
 * @returns {string} The skeleton grid HTML
 */
export function renderSkeletonGrid(count = 8) {
    const skeletonCard = `
        <article class="card skeleton-card">
            <div class="slide-preview skeleton-shimmer"></div>
            <div class="card-body">
                <div class="card-meta">
                    <span class="skeleton-shimmer" style="width: 50px; height: 18px; border-radius: 4px;"></span>
                    <span class="skeleton-shimmer" style="width: 70px; height: 14px;"></span>
                </div>
                <h3 class="card-title">
                    <span class="skeleton-shimmer" style="height: 1.1rem; width: 85%; display: block;"></span>
                </h3>
                <p class="card-snippet">
                    <span class="skeleton-shimmer" style="height: 0.85rem; width: 100%; display: block; margin-bottom: 6px;"></span>
                    <span class="skeleton-shimmer" style="height: 0.85rem; width: 70%; display: block;"></span>
                </p>
            </div>
        </article>
    `;
    return '<div class="grid">' + skeletonCard.repeat(count) + '</div>';
}
