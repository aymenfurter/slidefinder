...
export function updateFavoritesCount() {
    ...
    if (toggleBtn) {
        toggleBtn.style.display = favorites.length > 0 ? '' : 'none';
    }

    // If favorites view is open and becomes empty, return to main view.
    if (favorites.length === 0 && get('showingFavorites')) {
        toggleFavoritesView();
    }
}
