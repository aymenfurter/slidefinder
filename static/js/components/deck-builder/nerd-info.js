...
function renderPhaseEvent(event, container) {
    ...
    const phasePaths = {
        'init': 'services/deck_builder/service.py',
        'search': 'services/search_service.py',
        'outline': 'services/deck_builder/agents.py',
        'slide_selection': 'services/deck_builder/workflow.py',
        'complete': 'services/deck_builder/service.py'
    };
...
function renderSearchEvent(event, container) {
    ...
    el.innerHTML = `
        <div class="nerd-event-header">
            ...
            ${renderCodeLink('services/search_service.py')}
            <span class="nerd-event-time">${formatTime(event.timestamp)}</span>
        </div>
...
function renderLLMStartEvent(event, container) {
    ...
    const agentPaths = {
        'OutlineAgent': 'services/deck_builder/agents.py',
        'OfferAgent': 'services/deck_builder/executors/offer.py',
        'CritiqueAgent': 'services/deck_builder/executors/critique.py',
        'JudgeAgent': 'services/deck_builder/executors/judge.py'
    };
    const codePath = agentPaths[event.agent] || 'services/deck_builder/agents.py';
...
function renderSlideWorkflowStart(event, container) {
    ...
    const workflowLinks = [
        { name: 'workflow', path: 'services/deck_builder/workflow.py' },
        { name: 'search', path: 'services/deck_builder/executors/search.py' },
        { name: 'offer', path: 'services/deck_builder/executors/offer.py' },
        { name: 'critique', path: 'services/deck_builder/executors/critique.py' },
        { name: 'judge', path: 'services/deck_builder/executors/judge.py' }
    ];
...
function renderExecutorStart(event, container) {
    ...
    const executorPaths = {
        'search': 'services/deck_builder/executors/search.py',
        'offer': 'services/deck_builder/executors/offer.py',
        'critique': 'services/deck_builder/executors/critique.py',
        'judge': 'services/deck_builder/executors/judge.py'
    };
...
function renderEdgeTransition(event, container) {
    ...
    el.innerHTML = `
        <div class="nerd-edge-flow">
            ...
            ${renderCodeLink('services/deck_builder/workflow.py')}
        </div>
...
function renderWorkflowSearch(event, container) {
    ...
    el.innerHTML = `
        <div class="nerd-event-header">
            ...
            ${renderCodeLink('services/deck_builder/executors/search.py')}
            <span class="nerd-event-time">${formatTime(event.timestamp)}</span>
        </div>
        <div class="nerd-search-query">"${escapeHtml(event.query || '')}"</div>
        <div class="nerd-search-meta">
...
function renderSlideOffered(event, container) {
    ...
    el.innerHTML = `
        <div class="nerd-event-header">
            ...
            ${renderCodeLink('services/deck_builder/executors/offer.py')}
            <span class="nerd-event-time">${formatTime(event.timestamp)}</span>
        </div>
        ${thumb}
...
function renderSlideCritiqued(event, container) {
    ...
    el.innerHTML = `
        <div class="nerd-event-header">
            ...
            ${renderCodeLink('services/deck_builder/executors/critique.py')}
            <span class="nerd-event-time">${formatTime(event.timestamp)}</span>
        </div>
...
function renderJudgeInvoked(event, container) {
    ...
    el.innerHTML = `
        <div class="nerd-event-header">
            ...
            ${renderCodeLink('services/deck_builder/executors/judge.py')}
            <span class="nerd-event-time">${formatTime(event.timestamp)}</span>
        </div>
